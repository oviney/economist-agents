...
Every git operation returns a structured result the orchestrator can consume:

```json
{
  "operation": "commit|push|amend",
  "branch": "<branch-name>",
...
For multi-step workflows (Save Work, Double Commit Protocol), emit one record per step in order.
