...
*Auto-generated from data/skills_state/*.json and docs/ARCHITECTURE_PATTERNS.md on 2026-07-31 — regenerated in full each run; do not edit by hand.*

### Defect Prevention Patterns

#### Blind String Replace

**BUG-039** (high) - agent_sdk._shared.apply_editorial_fixes
- **Issue**: apply_editorial_fixes mangles Markdown image embeds: the exclamation-strip step turns '![alt](path)' into '.[alt](path)', breaking inline chart/hero embeds. Masked in hero mode by the auto-embed safety net running afterwards.
...
**BUG-026** (high) - governance
- **Issue**: B605: Command injection risk in governance.py - shell=True in subprocess call
- **Missed By**: unit_test
- **Prevention**:
  - Add security lint rules to CI/CD (Bandit)
  - Review all subprocess calls for shell=True
  - Add pre-commit security scan

**BUG-027** (medium) - featured_image_agent
- **Issue**: B113: requests.get() without timeout in featured_image_agent.py
- **Missed By**: unit_test
- **Prevention**:
  - Add timeout validation to all requests.get/post calls
  - Add linting rule for missing timeouts
  - Review all HTTP client calls

**BUG-032** (high) - research_tools
- **Issue**: scripts/arxiv_search.py module-level annotation `arxiv.Result` evaluated at class creation, crashes import when optional `arxiv` package missing
- **Prevention**:
  - Pattern: any module with `try: import optional_pkg / except ImportError: optional_pkg = None` must use `from __future__ import annotations` if it references the package in type annotations.

**BUG-033** (medium) - article_archive
- **Issue**: scripts/article_archive.py drops the injected ChromaDB client when CHROMADB_AVAILABLE is False, defeating dependency-injection in tests
- **Prevention**:
  - Pattern: dependency-injection points must run before optional-dependency guards.

**BUG-047** (high) - agent_sdk.stage3_runner.run_stage3 / _collect_text
- **Issue**: Keyless writer (Stage 3) exhausts its cumulative budget across retries and produces no article. A live keyless run of src.agent_sdk.pipeline hit BudgetExceededError (cap=$0.2169, cost_at_abort=$0.3166) after ~9 min with output/posts/ empty. The cumulative-budget-across-attempts logic plus repeated writer regeneration (_WRITER_MAX_ATTEMPTS) burned the budget, implying the writer is looping on malformed output rather than the budget merely being too low. DIAGNOSTIC PENDING: rerun with a generous --writer-budget to distinguish under-budget vs malformed-output loop. Related to the preamble/code-fence recovery in the closed PR #441 (_extract_article) and BUG-044.

**BUG-048** (medium) - agent_sdk.stage3_runner._collect_text
- **Issue**: Async-generator cleanup bug masks BudgetExceededError. When _raise_if_budget_exceeded fires inside _collect_text (stage3_runner.py:381->335), a 'RuntimeError: aclose(): asynchronous generator is already running' is raised during handling of the BudgetExceededError, so the clean typed budget error is obscured by an async cleanup RuntimeError. The query() async generator is not closed cleanly on the early raise.

**BUG-049** (medium) - scripts.arxiv_search._optimize_search_terms
- **Issue**: arXiv search-term optimizer garbles words. _optimize_search_terms (scripts/arxiv_search.py:302) lowercases the query then substring-replaces mapping keys with no word boundaries, so 'AI'->'artificial intelligence OR machine learning' fires inside 'drain' -> 'dr artificial intelligence OR machine learning n'. Observed live: query 'why flaky tests quietly drain engineering budgets' became '...quietly drartificial intelligence OR machine learningn engineering budgets', producing junk arXiv queries that return nothing.

#### Design Flaw

**BUG-038** (high) - _shared / economist_agent
- **Issue**: Pipeline quarantines an otherwise-publishable article for mechanically-fixable frontmatter defects (stale/missing date, missing categories, missing frontmatter block); LLM regeneration meant to fix them deleted the frontmatter entirely.
- **Missed By**: integration
- **Prevention**:
  - Harden apply_editorial_fixes to guarantee complete frontmatter when current_date is supplied
  - Wire finalizer into economist_agent.py before validation
...
**BUG-025** (high) - git_workflow
- **Issue**: Pre-commit hooks cause infinite commit loop blocking autonomous git operations
- **Missed By**: integration_test
- **Prevention**:
  - Added post-commit `git status --porcelain` check + `git add -u` + `git commit --amend --no-edit` in both deployment scripts

**BUG-034** (high) - quality_dashboard
- **Issue**: scripts/quality_dashboard.py reads sprint history from legacy `skills/sprint_history.json` instead of canonical `data/skills_state/sprint_history.json`, rendering dashboards without sprint trends
- **Prevention**:
...
**BUG-035** (high) - defect_tracker
- **Issue**: scripts/defect_tracker.py reads from legacy `skills/defect_tracker.json` instead of `data/skills_state/defect_tracker.json` — same migration miss as BUG-034
- **Prevention**:
  - See BUG-034 — same prevention applies.

**BUG-036** (high) - agent_metrics
- **Issue**: scripts/agent_metrics.py reads from legacy `skills/agent_metrics.json` instead of `data/skills_state/agent_metrics.json` — same migration miss as BUG-034/035
- **Prevention**:
  - See BUG-034 — same prevention applies.

**BUG-045** (high) - agent_sdk._shared / agent_sdk.tools.research_tools / scripts.arxiv_search
- **Issue**: arXiv provider contract mismatch: src/agent_sdk/_shared.py and src/agent_sdk/tools/research_tools.py both read insights.papers_analyzed from search_arxiv_for_topic(), but ArxivSearcher.extract_business_insights() never emitted that key, so arXiv silently contributed zero sources to every research brief. After #438 removed Brave/Google this left a single live research provider (Semantic Scholar) where the code believed it had two. Masked in CI because tests/test_research_tools.py mocked the arXiv return with a papers_analyzed dict the real code never produced.

**BUG-050** (high) - agent_sdk.pipeline research (deterministic) / empty-research guard
- **Issue**: Keyless deterministic research is unreliable and aborts the whole pipeline. The two free providers (arXiv, Semantic Scholar) rate-limit/err heavily (HTTP 429/503); when BOTH fail on a topic, deterministic research returns zero sources and the empty-research guard (#438) aborts the run (exit 2, 'Pipeline aborted: research failed', provider_failed={'arxiv':True,'semantic_scholar':True}). Observed live: a default-budget keyless run aborted before the writer. This is the primary blocker to reliable keyless article production — bigger than the writer bugs. Likely mitigation: default the keyless path to research_mode='claude_web' (ADR-0013, Claude's own WebSearch on the subscription — not the rate-limited academic APIs), which is exactly why claude_web was introduced.

#### Missing Field Hard Fail

**BUG-043** (medium) - agent_sdk.stage3_runner / chart_renderer
- **Issue**: When the graphics model omits the chart 'title' field, render_chart's _validate_spec raises ChartRenderError and crashes the whole pipeline. A missing title is mechanically recoverable and must not abort an otherwise-complete run.

#### Ordering

**BUG-040** (high) - agent_sdk.pipeline.run_pipeline
- **Issue**: run_pipeline chart_only strips the image: frontmatter (and its slug) BEFORE Stage 4, so _auto_embed_chart cannot derive the chart path and the required chart is never embedded -> publication_validator missing_chart. _run_resume already embeds-before-strips; run_pipeline does not.
...
**BUG-029** (high) - writer_agent
- **Issue**: Writer Agent generates articles 478-543 words (needs 800+ minimum)
- **Missed By**: integration_test
- **Prevention**:
  - Single source of truth: 700 minimum across all validators
...
**BUG-037** (high) - stage3_runner
- **Issue**: Stage 3 writer agent produces articles without image_alt or image_caption fields; publication validator now requires both as CRITICAL gates
- **Missed By**: integration_test
- **Prevention**:
  - Add image_alt and image_caption to Stage 3 structured output contract
  - Wire Claude vision call post-image-generation to ground fields against actual visual
  - Add integration test: stage3_runner output -> publication_validator passes image metadata gates

#### Single Shot No Retry

**BUG-044** (high) - agent_sdk.stage3_runner.run_stage3
- **Issue**: Stage 3 (run_stage3) generates the writer draft once and raises MalformedArticleError on any malformed output, aborting the whole run. The writer via the subscription CLI is non-deterministic (empty body, bare code fence), so a single bad draft kills an otherwise-keyless run. run_pipeline has no retry, unlike flow.py's revision loop.

#### Turn Cap Unhandled

**BUG-042** (high) - agent_sdk.stage3_runner._collect_text / research.claude_web
- **Issue**: Stage 3 graphics _collect_text call uses the default max_turns=1; against the real subscription CLI the graphics agent needs >1 turn and the SDK raises 'Reached maximum number of turns (1)', which _collect_text does not catch, crashing the whole pipeline. Also: claude_web research MAX_TURNS=12 is too low and soft-degrades to an empty brief on complex topics.

#### Unhandled Output Format

**BUG-041** (high) - agent_sdk.stage3_runner
- **Issue**: Stage 3 writer sometimes wraps the whole article in a Markdown code fence (```...```). The fence is not stripped, so the article does not start with '---' AND _strip_duplicate_article mistakes the real frontmatter (now at a non-zero offset) for a duplicate emission and deletes the entire body, raising MalformedArticleError (body_empty=True).
...
**BUG-030** (medium) - publication_validator
- **Issue**: Publication validator false positive on 'your-content' in filename (from actual title)
- **Missed By**: unit_test
- **Prevention**:
  - Enumerated YOUR-* placeholder tokens; no more wildcard match
  - Added regression test for title-derived slugs (your-content-strategy)
...
**visual_qa_improvement_trend** (medium)
