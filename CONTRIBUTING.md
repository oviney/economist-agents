...
## ✏️ Generating an article

One command, end to end, keyless. Stage 3 researches, writes, renders the chart,
and draws the hero SVG; Stage 4 polishes and validates.

```bash
IS_SANDBOX=1 python -m src.agent_sdk.pipeline "your noun-phrasey topic" \
    --research-mode claude_web
```

It writes:

- `output/posts/<slug>.md` — the finished, validated article
- `output/charts/<slug>.png` — the rendered chart (matplotlib, no API)
- `output/posts/images/<slug>-hero.svg` — the hero Claude drew (B-016b)
- `output/posts/<slug>.image_prompt.md` — the drawing brief, kept as a sidecar

Budget ~$1 and ~35 minutes per article (measured on the B-020 acceptance runs).

> **The two-step handshake is gone (B-021).** Stage 3 used to pause on exit 10
> for a hero image you generated by hand and dropped at
> `output/posts/images/<slug>.png`, then `--resume` ran Stage 4. B-016b made
> Claude draw the hero, so `--image-mode`, `--resume`, `--no-image`, exit codes
> 10/11, and the PNG gate were all removed. **To supply art by hand:** overwrite
> `output/posts/images/<slug>-hero.svg` (the sidecar prompt is the brief) and
> re-run.
...
> 10 and 11 were the handshake's codes. They are retired, not reused.
