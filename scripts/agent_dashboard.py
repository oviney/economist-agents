...
class AgentDashboard:
    ...
    def _generate_summary_table(self) -> List[str]:
        ...
        for agent_name in summary:
            ...
            costs = [r.get("cost_per_quality_unit", 0) for r in agent_runs 
                    if r.get("cost_per_quality_unit", 0) > 0 and r.get("cost_per_quality_unit", 0) != float('inf')]
        ...
        for agent_name, stats in summary.items():
            ...
            pass_rate = stats.get("validation_pass_rate", stats.get("avg_gate_pass_rate", 0))
            ...
            if pass_rate < self.CRITICAL_PASS_RATE:
                ...
                alert = " ⚠️"
            elif baseline_cost > 0 and avg_cost > baseline_cost * self.MAX_COST_MULTIPLIER:
                alert = " ⚠️"
        ...
        if baseline_cost > 0:
            lines.append("")
            lines.append(f"Baseline Cost/Quality: {baseline_cost:.0f} tokens per quality unit")
            lines.append(f"⚠️  = Warning (pass rate <{self.WARNING_PASS_RATE}% or cost >{self.MAX_COST_MULTIPLIER}x baseline)")
    ...
    def _generate_alerts(self) -> List[str]:
        ...
        for agent_name, stats in summary.items():
            ...
            if baseline_cost > 0 and avg_cost > baseline_cost * self.MAX_COST_MULTIPLIER:
    ...
    def _generate_agent_detail(self, agent_name: str) -> List[str]:
        ...
        if agent_name == "research_agent":
            lines.append(f"Verification Rate: {summary.get('avg_verification_rate', 0):.1f}%")
            lines.append(f"Validation Pass Rate: {summary.get('validation_pass_rate', 0):.1f}%")
            ...
            lines.append(f"Avg Regenerations: {summary.get('avg_regenerations', 0):.1f}")
            ...
            lines.append(f"Visual QA Pass Rate: {summary.get('avg_qa_pass_rate', 0):.1f}%")
        ...
        if len(agent_runs) >= 3:
            recent = agent_runs[-5:]
            recent_quality = [r.get("quality_score", 0) for r in recent if "quality_score" in r]
            if recent_quality:
                lines.append(f"\nRecent Performance (last {len(recent)} runs):")
                lines.append(f"  Quality Scores: {', '.join([str(int(q)) for q in recent_quality])}")
                ...
                if len(recent_quality) >= 3:
                    ...
                    if second_half > first_half + 5:
                        lines.append(f"  Trend: Improving ⬆️ (+{second_half - first_half:.1f} points)")
                    elif second_half < first_half - 5:
                        lines.append(f"  Trend: Declining ⬇️ ({second_half - first_half:.1f} points)")
                    else:
                        lines.append(f"  Trend: Stable ➡️")
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if agent_scores:
            ...
            if worst_score < self.WARNING_PASS_RATE:
                ...
                recommendations.append(
                    f"   - Review agent prompt for clarity"
                )
                recommendations.append(
                    f"   - Add explicit quality gate checks"
                )
                recommendations.append(
                    f"   - Enhance self-validation logic"
                )
        ...
        if high_rework_agents:
            recommendations.append("")
            recommendations.append(
                f"2. Reduce rework for agents with >30% regeneration rate:"
            )
