...
class AgentReviewer:
    ...
    def review_research_output(self, research_data: dict) -> tuple[bool, list[str]]:
        ...
        if "data_points" in research_data:
            ...
            if isinstance(data_points, list) and len(data_points) > 0:
                verified_count = sum(
                    1 for dp in data_points if dp.get("verified", False)
                )
    ...
    def review_writer_output(
        ...
        body = (
            article_content.split("---", 2)[2].strip()
            if article_content.count("---") >= 2
            else ""
        )
    ...
    def review_editor_output(self, edited_article: str) -> tuple[bool, list[str]]:
        ...
        # Extract body
        body = (
            edited_article.split("---", 2)[2].strip()
            if edited_article.count("---") >= 2
            else ""
        )
        ...
        for pattern in (
            self.banned_openings + self.banned_phrases + self.banned_closings
        ):
