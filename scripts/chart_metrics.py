...
class ChartMetricsCollector:
    ...
    def start_chart(self, chart_title: str, chart_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def record_generation(self, chart_record: Dict[str, Any], success: bool, error: str = None):
        """Record chart generation result"""
        chart_record["generation_time_seconds"] = time.time() - chart_record["start_time"]
        chart_record["generation_success"] = success
        
        if success:
            self.metrics["summary"]["total_charts_generated"] += 1
            self.metrics["summary"]["total_generation_time_seconds"] += chart_record["generation_time_seconds"]
        else:
            if error:
    ...
    def end_session(self):
        ...
        session_summary = {
            ...
            "charts_generated": len([c for c in self.current_session["charts"] if c.get("generation_success")]),
            "visual_qa_runs": len([c for c in self.current_session["charts"] if c.get("visual_qa_run")]),
            "visual_qa_passed": len([c for c in self.current_session["charts"] if c.get("visual_qa_passed")]),
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if summary["total_charts_generated"] > 0:
            summary["avg_generation_time_seconds"] = (
                summary["total_generation_time_seconds"] / summary["total_charts_generated"]
            )
        else:
    ...
    def export_report(self, include_sessions: bool = False) -> str:
        ...
        if top_patterns:
            for i, pattern in enumerate(top_patterns, 1):
                ...
                report_lines.append(f"      First: {pattern['first_seen'][:10]}, Last: {pattern['last_seen'][:10]}")
        else:
        ...
        if include_sessions and self.metrics["sessions"]:
            ...
            for session in self.metrics["sessions"][-5:]:  # Last 5 sessions
                ...
                report_lines.append(f"    Visual QA: {session['visual_qa_passed']}/{session['visual_qa_runs']} passed")
