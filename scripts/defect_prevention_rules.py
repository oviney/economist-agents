...
class DefectPrevention:
    ...
    def check_all(self, article_content: str, metadata: Dict[str, Any] = None) -> List[str]:
        """
        ...
        for rule in self.rules:
            ...
            if violation:
                violations.append(f"[{rule['severity'].upper()}] {violation} (Pattern: {rule['id']})")
    ...
    def _check_chart_embedding(self, content: str, metadata: Dict) -> str:
        ...
        has_reference = any(re.search(pattern, content, re.IGNORECASE) 
                           for pattern in chart_references)
    ...
    def _check_category_present(self, content: str, metadata: Dict) -> str:
        ...
        has_category = bool(re.search(r'^(category|categories):', frontmatter, re.MULTILINE))
    ...
    def generate_report(self) -> str:
        ...
        report = [
            ...
            f"**Source**: 6 bugs with Root Cause Analysis",
            "",
        ...
        for rule in self.rules:
            ...
            report.append(f"**Prevention Actions**:")
...
"""
    violations = checker.check_all(test_article, {"chart_data": {"title": "Test Chart"}})
    if violations:
        print("  ❌ CAUGHT:")
        for v in violations:
            print(f"    {v}")
...
"""
    violations = checker.check_all(test_article_good, {"chart_data": {"title": "Test Chart"}})
    if violations:
        print("  ❌ VIOLATIONS:")
        for v in violations:
            print(f"    {v}")
