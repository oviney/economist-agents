...
class DefectTracker:
    ...
    def fix_bug(self, bug_id: str, fix_commit: str, notes: str = None,
                prevention_test_added: bool = False,
                prevention_test_file: str = None) -> None:
    ...
    def update_bug_rca(self, bug_id: str, root_cause: str = None,
                      root_cause_notes: str = None,
                      introduced_in_commit: str = None,
                      introduced_date: str = None,
                      missed_by_test_type: str = None,
    ...
    def _update_summary(self) -> None:
        ...
        bugs_with_ttd = [b for b in self.tracker["bugs"] 
                        if b.get("time_to_detect_days") is not None]
        bugs_with_ttr = [b for b in self.tracker["bugs"] 
                        if b.get("time_to_resolve_days") is not None]
        
        avg_ttd = (sum(b["time_to_detect_days"] for b in bugs_with_ttd) / len(bugs_with_ttd)) \
                  if bugs_with_ttd else None
        avg_ttr = (sum(b["time_to_resolve_days"] for b in bugs_with_ttr) / len(bugs_with_ttr)) \
                  if bugs_with_ttr else None
        
        # NEW: Critical bug TTD
        critical_ttd = [b["time_to_detect_days"] for b in bugs_with_ttd 
    ...
    def generate_report(self) -> str:
        ...
        if metrics.get("root_cause_distribution"):
            ...
            sorted_causes = sorted(metrics["root_cause_distribution"].items(), 
                                  key=lambda x: x[1], reverse=True)
            for cause, count in sorted_causes:
                pct = (count / metrics["total_bugs"] * 100) if metrics["total_bugs"] > 0 else 0
                report.append(f"- **{cause.replace('_', ' ').title()}**: {count} ({pct:.0f}%)")
        ...
        if metrics.get("test_gap_distribution"):
            ...
            sorted_gaps = sorted(metrics["test_gap_distribution"].items(), 
                               key=lambda x: x[1], reverse=True)
            for gap, count in sorted_gaps:
                pct = (count / metrics["total_bugs"] * 100) if metrics["total_bugs"] > 0 else 0
                report.append(f"- **{gap.replace('_', ' ').title()}**: {count} ({pct:.0f}%)")
        ...
        if metrics.get("avg_time_to_detect_days") is not None:
            ...
            if metrics.get("avg_critical_ttd_days") is not None:
                report.append(f"- **Avg Critical Bug TTD**: {metrics['avg_critical_ttd_days']} days")
            if metrics.get("avg_time_to_resolve_days") is not None:
                report.append(f"- **Avg Time to Resolve**: {metrics['avg_time_to_resolve_days']} days")
        ...
        if production_bugs:
            ...
            for bug in production_bugs:
                ...
                if bug.get("root_cause"):
                    report.append(f"  - Root Cause: {bug['root_cause'].replace('_', ' ')}")
                if bug.get("time_to_detect_days") is not None:
                    report.append(f"  - Time to Detect: {bug['time_to_detect_days']} days")
