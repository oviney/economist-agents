...
class EditorAgentDiagnostic:
    ...
    def __init__(self, test_articles_dir: str = None):
        self.test_articles_dir = Path(test_articles_dir) if test_articles_dir else Path("output")
    ...
    def analyze_historical_performance(self) -> Dict:
        ...
        gate_pass_rate = (avg_gates_passed / total_gates) * 100 if total_gates > 0 else 0
        ...
        if len(editor_metrics) >= 3:
            ...
            first_half_rate = (sum(m["gates_passed"] for m in first_half) / (len(first_half) * total_gates)) * 100
            second_half_rate = (sum(m["gates_passed"] for m in second_half) / (len(second_half) * total_gates)) * 100
    ...
    def analyze_pattern_detection_failures(self, test_articles: List[Path] = None) -> Dict:
        ...
        total_violations = failures["openings"] + failures["phrases"] + failures["closings"] + failures["verification_flags"]
        failure_rate = (total_violations / (failures["total_articles"] * 4)) * 100 if failures["total_articles"] > 0 else 0
    ...
    def identify_root_causes(self) -> Dict:
        ...
        if self.findings.get("historical_performance", {}).get("avg_gates_failed", 0) > 1:
            root_causes.append({
    ...
    def propose_remediation_options(self) -> List[Dict]:
        ...
        options.append({
            ...
            "targets": ["prompt_drift", "pattern_detection_gaps", "gate_ambiguity", "llm_model_changes"]
        })
    ...
    def generate_diagnosis_report(self, output_path: str = "docs/EDITOR_AGENT_DIAGNOSIS.md"):
        ...
        report_lines = [
            ...
            f"**Sprint**: Sprint 7, Story 1 (P0)",
            f"**Goal**: Identify root causes of Editor Agent quality decline",
            "",
            "---",
            "",
            "## Executive Summary",
        ...
        if "historical_performance" in self.findings:
            ...
            if perf.get("decline_detected"):
                report_lines.append(f"⚠️  **DECLINE DETECTED**: {perf.get('decline_percentage', 0):.1f}% drop in performance over time")
            else:
                report_lines.append(f"✅ **Performance Stable**: No significant decline detected (±{abs(perf.get('decline_percentage', 0)):.1f}%)")
            report_lines.append("")
        ...
        report_lines.extend([
            ...
            "",
            "**Openings**: " + ", ".join([f"`{p}`" for p in self.BANNED_PATTERNS["openings"][:3]]) + "...",
            "",
            "**Phrases**: " + ", ".join([f"`{p}`" for p in self.BANNED_PATTERNS["phrases"]]),
            "",
            "**Closings**: " + ", ".join([f"`{p}`" for p in self.BANNED_PATTERNS["closings"][:3]]) + "...",
            "",
            "---",
            "",
            f"**Report Generated**: {datetime.now().isoformat()}",
            f"**Diagnostic Tool**: `scripts/editor_agent_diagnostic.py`",
            ""
        ])
        ...
        print(f"   Sections: Executive Summary, Root Causes, Remediation Options, Recommendations")
    ...
    def run_full_diagnostic(self) -> Dict:
        ...
        print(f"\n📄 Report: {report_path}")
        print("\n📋 Findings Summary:")
        print(f"   Root Causes: {self.findings.get('root_causes', {}).get('root_causes_identified', 0)}")
        print(f"   Remediation Options: {len(self.findings.get('remediation_options', []))}")
...
def main():
    ...
    if args.analyze or not args.report:
        ...
        print(f"   [{'✅' if results['status'] == 'complete' else '❌'}] Diagnostic suite created")
        print(f"   [{'✅' if 'historical_performance' in results['findings'] else '❌'}] Baseline metrics established")
        print(f"   [{'✅' if 'root_causes' in results['findings'] else '❌'}] Root causes identified")
        print(f"   [{'✅' if 'remediation_options' in results['findings'] else '❌'}] 3 remediation options proposed")
        print(f"   [✅] EDITOR_AGENT_DIAGNOSIS.md published")
    elif args.report:
