...
def generate_sample_data():
        ...
        agent_metrics["runs"].append({
            ...
            "agents": {
                ...
                "writer_agent": {
                    ...
                    "actual": "Pass" if banned_phrases == 0 else "Needs improvement"
                },
                "editor_agent": {
                    ...
                    "quality_issues": [] if gates_failed == 0 else ["Style", "Structure"],
    ...
    agent_metrics["summary"]["agents"]["research_agent"] = {
        "total_runs": 6,
        "avg_verification_rate": round(sum(r["agents"]["research_agent"]["verification_rate"] 
                                          for r in agent_metrics["runs"]) / 6, 1),
        "trend": "improving ⬆️"
    }
    
    agent_metrics["summary"]["agents"]["writer_agent"] = {
        "total_runs": 6,
        "clean_draft_rate": round(sum(1 for r in agent_metrics["runs"] 
                                     if r["agents"]["writer_agent"]["banned_phrases"] == 0) / 6 * 100, 1),
        "avg_regenerations": round(sum(r["agents"]["writer_agent"]["regenerations"] 
                                      for r in agent_metrics["runs"]) / 6, 1)
    }
    
    agent_metrics["summary"]["agents"]["editor_agent"] = {
        "total_runs": 6,
        "avg_gate_pass_rate": round(sum(r["agents"]["editor_agent"]["gate_pass_rate"] 
                                       for r in agent_metrics["runs"]) / 6, 1),
        "trend": "improving ⬆️"
    }
    ...
    agent_metrics["summary"]["agents"]["graphics_agent"] = {
        "total_runs": 6,
        "avg_qa_pass_rate": round(sum(r["agents"]["graphics_agent"]["visual_qa_pass_rate"] 
                                     for r in agent_metrics["runs"]) / 6, 1),
        "avg_violations": round(sum(r["agents"]["graphics_agent"]["zone_violations"] 
                                   for r in agent_metrics["runs"]) / 6, 1)
    }
    ...
    print(f"  Research Agent: {agent_metrics['summary']['agents']['research_agent']['avg_verification_rate']}% verification")
    print(f"  Writer Agent: {agent_metrics['summary']['agents']['writer_agent']['clean_draft_rate']}% clean drafts")
    print(f"  Editor Agent: {agent_metrics['summary']['agents']['editor_agent']['avg_gate_pass_rate']}% gate pass rate")
    print(f"  Graphics Agent: {agent_metrics['summary']['agents']['graphics_agent']['avg_qa_pass_rate']}% QA pass rate")
