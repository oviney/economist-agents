...
class GitHubSprintSync:
    ...
    def parse_sprint_stories(self, sprint_number: int) -> List[Dict]:
        ...
        for match in re.finditer(story_pattern, sprint_section):
            ...
            story_end = story_start + next_story.start() if next_story else len(sprint_section)
    ...
    def create_github_issue_from_story(self, story: Dict, sprint_number: int) -> int:
        ...
        labels = ['sprint-story', f"sprint-{sprint_number}", f"{story['points']}-points"]
    ...
    def push_to_github(self, sprint_number: int = 2):
        ...
        for story in stories:
            print(f"  Creating issue for Story {story['number']}: {story['title'][:50]}...")
    ...
    def show_github_status(self):
        ...
        for issue in issues:
            ...
            sprint_label = next((l.name for l in issue.labels if l.name.startswith('sprint-')), None)
            if not sprint_label:
                continue
            ...
            points_label = next((l.name for l in issue.labels if '-points' in l.name), None)
...
def main():
    ...
    parser.add_argument('--push-to-github', action='store_true',
                       help='Create GitHub issues from SPRINT.md')
    parser.add_argument('--pull-from-github', action='store_true',
                       help='Update SPRINT.md from GitHub issues')
    parser.add_argument('--show-github-status', action='store_true',
                       help='Show sprint status from GitHub')
    parser.add_argument('--sprint', type=int, default=2,
                       help='Sprint number (default: 2)')
