...
class GovernanceTracker:
    ...
    def request_approval(self, stage: str, summary: str, details: Dict = None) -> bool:
        ...
        if stage.lower().replace(' ', '_') in [a.lower() for a in self.agent_outputs]:
            agent_key = [k for k in self.agent_outputs.keys() 
                        if k.lower() == stage.lower().replace(' ', '_')][0]
    ...
    def log_decision(self, decision_type: str, choice: str, rationale: str, data: Dict = None):
...
class InteractiveMode:
    ...
    def select_option(prompt: str, options: list) -> int:
        ...
        while True:
            try:
                choice = input("\nSelect option [1-{}]: ".format(len(options))).strip()
                idx = int(choice) - 1
