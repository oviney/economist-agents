...
Deliberately narrow: `ruff check` over dirty tracked Python files. Not the test suite —
that is slower and noisier than a Stop gate should be. Widening it to tests is a one-line
change and an open question in `docs/specs/harness-engineering.md`.
"""
...
def handle(
    ...
    Args:
        ...
        violations: Pre-computed violation text; collected from ruff when omitted.
    ...
    detail = collect_violations() if violations is None else violations
    if not detail:
        return {}
    ...
    return {
        "decision": "block",
        "reason": (
            "Harness Stop gate: ruff is red on Python files you changed. Fix these "
            "before finishing the turn — `make ci-local` is the merge gate and will "
            "reject them anyway, and you have the context now that the owner will not "
            "have later.\n\n"
            f"{detail}\n\n"
