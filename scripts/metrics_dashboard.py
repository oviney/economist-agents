...
class MetricsDashboard:
    ...
    def _show_agent_detail(self, agent_name, stats):
        ...
        runs = [
            run for run in self.agent_metrics["runs"] if agent_name in run["agents"]
        ]
    ...
    def show_prediction_accuracy(self):
        ...
        for agent_name, stats in agent_accuracy.items():
            ...
            print(
                f"{agent_name.replace('_', ' ').title():20s} │ {accuracy:5.1f}% │ {bar}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="View quality and agent performance metrics"
    )
    ...
    parser.add_argument(
        "--accuracy", action="store_true", help="Show prediction accuracy"
    )
