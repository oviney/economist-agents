...
class GitHubIssueCreator:
    ...
    def create_all(self, items: List[Dict[str, Any]], dry_run: bool = False, 
                   skip_completed: bool = True) -> List[str]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Migrate BACKLOG.md to GitHub Issues'
    )
    parser.add_argument(
        '--backlog',
    ...
    if args.dry_run or args.create:
        ...
        print("\n" + "="*60)
        if args.dry_run:
            print(f"📊 Would create {len([i for i in items if i['status'] != 'complete' or args.include_completed])} issues")
