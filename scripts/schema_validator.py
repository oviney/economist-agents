...
class FrontMatterValidator:
    ...
    def validate(self, content: str) -> Tuple[bool, List[str]]:
        ...
        if len(parts) < 3:
            issues.append("CRITICAL: Incomplete YAML front matter (missing closing ---)")
            return False, issues
        ...
        if 'date' in front_matter:
            ...
            if not re.match(r'^\d{4}-\d{2}-\d{2}$', date_str):
                issues.append(
                    f"CRITICAL: Invalid date format '{date_str}' "
        ...
        if 'categories' in front_matter:
            ...
            if not isinstance(categories, list):
                ...
                issues.append(
                    f"WARNING: Too many categories ({len(categories)}, "
                    f"≤3 recommended)"
                )
            else:
                # Validate each category
                allowed_categories = self.schema["properties"]["categories"]["items"]["enum"]
        ...
        if 'ai_assisted' not in front_matter:
            ...
            body = parts[2].lower()
            ai_mentions = ['ai', 'llm', 'gpt', 'claude', 'generated', 'artificial intelligence']
...
def validate_front_matter(content: str, expected_date: str = None) -> Tuple[bool, List[str]]:
    """
