...
def get_api_key(provider: str = 'openai') -> str:
    ...
    if provider == 'openai' and not api_key.startswith('sk-'):
        raise ValueError(f"[SECURITY] Invalid OpenAI API key format")
    elif provider == 'anthropic' and not api_key.startswith('sk-ant-'):
        raise ValueError(f"[SECURITY] Invalid Anthropic API key format")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"ℹ️  Anthropic key not set (optional)")
