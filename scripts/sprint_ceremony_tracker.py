...
class SprintCeremonyTracker:
    ...
    def end_sprint(self, sprint_number: int) -> None:
        ...
        print(f"⚠️  Next: Complete retrospective before starting Sprint {sprint_number + 1}")
    ...
    def can_start_sprint(self, sprint_number: int) -> bool:
        ...
        if prev_sprint_key not in self.tracker["sprints"]:
            ...
            print(f"   Run: python3 sprint_ceremony_tracker.py --end-sprint {sprint_number - 1}")
            return False
        ...
        if not prev_sprint.get("retrospective_done", False):
            ...
            print(f"   Run: python3 sprint_ceremony_tracker.py --retrospective {sprint_number - 1}")
            return False
        
        if not prev_sprint.get("backlog_refined", False):
            print(f"❌ BLOCKED: Sprint {sprint_number} backlog not refined")
            print(f"   Run: python3 sprint_ceremony_tracker.py --refine-backlog {sprint_number}")
            return False
        
        if not prev_sprint.get("next_sprint_dor_met", False):
            print(f"❌ BLOCKED: Sprint {sprint_number} Definition of Ready not met")
            print(f"   Run: python3 sprint_ceremony_tracker.py --validate-dor {sprint_number}")
            return False
    ...
    def complete_retrospective(self, sprint_number: int) -> str:
        ...
        self.tracker["sprints"][sprint_key]["retrospective_date"] = datetime.now().isoformat()
        self.save()
...
"""
    ...
    def complete_backlog_refinement(self, sprint_number: int) -> str:
        ...
        self.tracker["sprints"][sprint_key]["refinement_date"] = datetime.now().isoformat()
        self.save()
...
"""
    ...
    def validate_dor(self, sprint_number: int) -> tuple[bool, List[str]]:
        ...
        if sprint_number > 1:
            ...
            if prev_sprint_key in self.tracker["sprints"]:
                if not self.tracker["sprints"][prev_sprint_key].get("retrospective_done", False):
        ...
        if not missing:
            ...
            if sprint_number > 1:
                prev_sprint_key = f"sprint_{sprint_number - 1}"
                if prev_sprint_key in self.tracker["sprints"]:
                    self.tracker["sprints"][prev_sprint_key]["next_sprint_dor_met"] = True
            self.save()
            return True, []
    ...
    def generate_report(self) -> str:
        ...
        # List all sprints
        for sprint_key in sorted(self.tracker["sprints"].keys(), 
            ...
            report.append(f"  Backlog Refined: {'✅ Done' if backlog else '❌ Not done'}")
        ...
        if current:
            ...
            if current_sprint:
                if not current_sprint.get("retrospective_done", False):
                    ...
                    report.append(f"     Run: python3 sprint_ceremony_tracker.py --retrospective {current}")
                elif not current_sprint.get("backlog_refined", False):
                    report.append(f"  1. Refine Sprint {current + 1} backlog")
                    report.append(f"     Run: python3 sprint_ceremony_tracker.py --refine-backlog {current + 1}")
                elif not current_sprint.get("next_sprint_dor_met", False):
                    report.append(f"  1. Validate Sprint {current + 1} Definition of Ready")
                    report.append(f"     Run: python3 sprint_ceremony_tracker.py --validate-dor {current + 1}")
                else:
                    report.append(f"  ✅ All ceremonies complete - Sprint {current + 1} ready to start")
...
Examples:
  ...
  python3 sprint_ceremony_tracker.py --report
    ...
    parser.add_argument('--end-sprint', type=int, metavar='N',
                       help='Mark sprint N as complete')
    parser.add_argument('--can-start', type=int, metavar='N',
                       help='Check if sprint N can start')
    parser.add_argument('--retrospective', type=int, metavar='N',
                       help='Complete retrospective for sprint N')
    parser.add_argument('--refine-backlog', type=int, metavar='N',
                       help='Complete backlog refinement for sprint N')
    parser.add_argument('--validate-dor', type=int, metavar='N',
    ...
    if args.end_sprint:
        ...
        if is_valid:
            ...
            print(f"   All 8 DoR criteria passed")
