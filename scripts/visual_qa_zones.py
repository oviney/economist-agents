...
class ZoneBoundaryValidator:
    ...
    def validate_chart(self, chart_path: str) -> tuple[bool, list[str]]:
        ...
        if not chart_file.exists():
            self.issues.append(f"Chart file not found: {chart_file}")
        ...
        if not self._validate_filename(chart_file):
            self.issues.append(
                f"Invalid filename: {chart_file.name}. Must be slug-style (lowercase-with-hyphens.png)"
            )

        # Check 2: Look for corresponding generation script or metadata
        script_path = (
            chart_file.parent.parent
            / "scripts"
            / chart_file.name.replace(".png", ".py")
        )
        ...
        try:
            ...
            pixel_issues = self._validate_pixels(chart_file)
