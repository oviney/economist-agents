...
class ZoneBoundaryValidator:
    ...
    def validate_chart(self, chart_path: str) -> Tuple[bool, List[str]]:
        ...
        if not self._validate_filename(chart_path):
            self.issues.append(f"Invalid filename: {chart_path.name}. Must be slug-style (lowercase-with-hyphens.png)")
        
        # Check 2: Look for corresponding generation script or metadata
        script_path = chart_path.parent.parent / "scripts" / chart_path.name.replace('.png', '.py')
    ...
    def _validate_matplotlib_code(self, script_path: Path) -> List[str]:
        ...
        title_matches = re.findall(r'fig\.text\([^,]+,\s*(0\.\d+),.*?["\'].*?title', code, re.IGNORECASE)
        ...
        subtitle_matches = re.findall(r'fig\.text\([^,]+,\s*(0\.\d+),.*?["\'].*?subtitle', code, re.IGNORECASE)
        ...
        source_matches = re.findall(r'fig\.text\([^,]+,\s*(0\.\d+),.*?["\'].*?[Ss]ource', code)
        ...
        for match in annotate_matches:
            if 'xytext' not in match:
                issues.append(f"Inline label missing xytext offset (will overlap data): {match[:50]}...")
    ...
    def _validate_pixels(self, chart_path: Path) -> List[str]:
        ...
        try:
            ...
            if bg_matches.sum() < (bg_pixels.shape[0] * bg_pixels.shape[1] * 0.3):
                issues.append("Background color #f1f0e9 not dominant (expected warm beige)")
    ...
    def generate_report(self) -> str:
        ...
        report = [
            "❌ Zone Boundary Violations Detected:",
            ""
        ]
