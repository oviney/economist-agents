...
"""
    ...
    banned_found = {
        "opening": any("opening contains" in i.lower() and "banned" in i.lower() for i in issues),
        "phrases": any("game-changer" in i.lower() for i in issues),
        "verification": any("verification flags" in i.lower() for i in issues),
        "closing": any("ending contains" in i.lower() and "banned" in i.lower() for i in issues),
    }
...
def test_research_agent_validation():
    ...
    verification_issues = [i for i in issues if "verification" in i.lower() or "source" in i.lower()]
...
def test_chart_visual_bug_title_overlap():
    ...
    assert chart_metadata['title_y_position'] >= 0.96, "Title intrudes into red bar zone"
...
def test_chart_visual_bug_xaxis_intrusion():
    ...
    print(f"   - X-axis zone: {label_metadata['xaxis_zone_start']}-{label_metadata['xaxis_zone_end']}")
...
"""
    ...
    assert len(critical_review) == 0, f"Should have no critical review issues, got: {critical_review}"
